...
public class PatientAERS extends JFrame implements MouseListener {

    JLabel NameL, usernameL, addressL, phoneNumberL, passwordL, heightL, weightL, ageL, genderL;

    JTextField NameT, usernameT, addressT, phoneNumberT, heightT, weightT, ageT, genderT;
    JPasswordField passwordT ;

    JButton addB, searchB, viewB, deleteB, editB, backupB;
    JPanel p1, p2, mainP;

    public PatientAERS() throws ParseException {
...
//               JOptionPane.showMessageDialog(null, listofPat.get(x), "list ", JOptionPane.WARNING_MESSAGE);
    ...
    public class ButtonWatcher implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (!"".equals(NameT.getText()) && !"".equals(heightT.getText()) && !"".equals(weightT.getText()) && !"".equals(ageT.getText()) && !"".equals(genderT.getText()) && !"".equals(usernameT.getText()) && !"".equals(passwordT.getText()) && !"".equals(phoneNumberT.getText())) {
                try {
                ...
                p.setPhone(phoneNumberT.getText());
                p.setHeight(heightT.getText());
                p.setWeight(weightT.getText());
                p.setName(NameT.getText());
                p.setAge(ageT.getText());
                p.setGender(genderT.getText());
                p.setPassword(passwordT.getText());
                listofPat.add(p);
                if (listofPat.contains(p)) {
